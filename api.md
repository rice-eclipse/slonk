...
- _Sensor group_: A sensor group is a set of sensors which are sampled at the
...
- `frequency_standby` - number: The number of times, per second, to sample all
  the sensors in the sensor group _outside_ of ingition procedures.
...
_Up for discussion_: allow for different calibration modes than just a linear
one?
